...
@interface YYAnimatedImageView() {
    ...
    dispatch_semaphore_t _lock; ///< lock for _buffer 信号量锁（用于 _buffer）
    NSOperationQueue *_requestQueue; ///< image request queue, serial 图片请求队列，串行
    
    CADisplayLink *_link; ///< ticker for change frame 帧转换
    NSTimeInterval _time; ///< time after last frame 上一帧之后的时间
    
    UIImage *_curFrame; ///< current frame to display 当前帧
    NSUInteger _curIndex; ///< current frame index (from 0) 当前帧索引
    NSUInteger _totalFrameCount; ///< total frame count 帧总数
    
    BOOL _loopEnd; ///< whether the loop is end. 循环是否end
    NSUInteger _curLoop; ///< current loop count (from 0) 当前循环次数
    NSUInteger _totalLoop; ///< total loop count, 0 means infinity 总循环次数, 0 表示无穷
    
    NSMutableDictionary *_buffer; ///< frame buffer 帧缓冲区
    BOOL _bufferMiss; ///< whether miss frame on last opportunity 是否丢帧，在上面 _link 定时执行的 step 函数中从帧缓冲区读取下一帧图片时如果没读到，则视为丢帧
    NSUInteger _maxBufferCount; ///< maximum buffer count 最大缓冲计数
    NSInteger _incrBufferCount; ///< current allowed buffer count (will increase by step) 当前允许的缓存区计数（将逐步增加）
    
    CGRect _curContentsRect; /// 针对 YYSpriteSheetImage
    BOOL _curImageHasContentsRect; ///< image has implementated "animatedImageContentsRectAtIndex:" 图像类是否实现了 animatedImageContentsRectAtIndex: 接口
}
